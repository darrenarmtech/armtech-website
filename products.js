// Product data - Replace with your actual 102 products
const products = [
    // Fluke products (43)
    ...
    // Lutron products (sample - add your full 99)
    ...
    // Hanna products (sample - add your full 32)
    ...
    // Testo products (sample - add your full 38)
...
// Brand logos
const brandLogos = {
    fluke: 'https://drive.google.com/uc?id=15LJElHulkON-boZhHGeODjhN3KtPh9FW',
    lutron: 'https://drive.google.com/uc?id=1Jn8pjeAW-nxmokxkVQacservccj_BLQf',
    hanna: 'https://drive.google.com/uc?id=1V7ssFoJwY6tsOm5y-p4rl000GJ7Xapfu',
    testo: '' // Add Testo logo when available
};
...
function renderProducts(brandFilter) {
    ...
    grid.innerHTML = filtered.map(product => `
        <div class="product-card ${quoteCart.includes(product.id) ? 'in-quote' : ''}" 
             onclick="toggleQuote(${product.id})" 
             data-product-id="${product.id}">
            ${brandLogos[product.brand] ? 
                `<img src="${brandLogos[product.brand]}" alt="${product.brand}" class="brand-logo">` : 
...
// Toggle product in quote cart
function toggleQuote(productId) {
    const index = quoteCart.indexOf(productId);
    if (index > -1) {
        quoteCart.splice(index, 1);
    } else {
        quoteCart.push(productId);
    }
    saveQuoteCart();
    updateQuoteUI();
    renderProducts(getActiveBrand());
}

// Get current active brand filter
function getActiveBrand() {
    const activeBtn = document.querySelector('.filter-btn.active');
    return activeBtn ? activeBtn.dataset.brand : 'all';
}

// Update quote button UI
function updateQuoteUI() {
    const countEl = document.getElementById('quoteCount');
    const quoteBtn = document.getElementById('quoteBtn');
    
    if (quoteCart.length > 0) {
        countEl.textContent = quoteCart.length;
        countEl.style.display = 'flex';
        
        // Update WhatsApp link
        const selectedProducts = products.filter(p => quoteCart.includes(p.id));
        const message = `Hi, I would like to request a quote for the following products:\\n\\n${selectedProducts.map(p => `• ${p.name} (${p.sku})`).join('\\n')}`;
        quoteBtn.href = `https://wa.me/6591234567?text=${encodeURIComponent(message)}`;
    } else {
        countEl.style.display = 'none';
        quoteBtn.href = 'https://wa.me/6591234567?text=I%20want%20to%20request%20a%20quote';
    }
}

// Save quote cart to localStorage
function saveQuoteCart() {
    localStorage.setItem('armtechQuoteCart', JSON.stringify(quoteCart));
}

// Load quote cart from localStorage
function loadQuoteCart() {
    const saved = localStorage.getItem('armtechQuoteCart');
    if (saved) {
