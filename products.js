...
function toggleQuote(productId) {
    ...
    if (index > -1) quoteCart.splice(index, 1);
    else quoteCart.push(productId);
    saveQuoteCart();
    updateQuoteUI();
...
function saveQuoteCart() { try { localStorage.setItem('armtechQuote', JSON.stringify(quoteCart)); } catch(e) {} }
function loadQuoteCart() { try { const s = localStorage.getItem('armtechQuote'); if (s) { quoteCart = JSON.parse(s); updateQuoteUI(); } } catch(e) {} }
